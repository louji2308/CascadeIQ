...
const { runQuery } = require('./neo4j'); // we'll move neo4j.js here
...
app.get('/api/scenarios', async(req, res) => {
    try {
      ...
      RETURN s.id AS id, s.name AS name, s.year AS year,
             s.location AS location, s.deaths AS deaths,
...
// Cascade endpoint (with removed nodes support)
app.get('/api/cascade/:hazardId', async(req, res) => {
    try {
      ...
      RETURN
        [n IN ns | {id: n.id, name: n.name, label: labels(n)[0], severity: n.severity}] AS nodes,
